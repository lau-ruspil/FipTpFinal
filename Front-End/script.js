...
document.getElementById("nombreCliente").addEventListener("input", (e) => {
	...
	if (nombre.value === "") {
		ocultarError(nombre); // Si el campo está vacío, eliminamos el error
...
formularioVisita.addEventListener("submit", (e) => {
	...
	hoy.setHours(0, 0, 0, 0); // Asegurar que la hora de "hoy" sea las 00:00 para una comparación exacta
...
document.getElementById("fechaVisita").addEventListener("input", (e) => {
	...
	// Ajustar ambas fechas a las 00:00 para compararlas solo por día
	...
	else if (isNaN(fechaIngresada)) {
		mostrarError(fechaInput, "Fecha no válida");
	}
